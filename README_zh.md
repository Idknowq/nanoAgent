...
当前阶段：基于 asyncio 的工具调用循环，支持运行持久化、缓存友好的 Prompt 设计、一层子
Agent 委托，并正在清理剩余的同步持久化边界。
...
部分文件持久化模块仍通过临时 async boundary 包装同步原子写。Step 12 的目标是在不改变运行产物格式和崩溃恢复语义的前提下，清理这些同步残留。
