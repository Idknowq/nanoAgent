...
def test_resolve_workspace_path_rejects_empty_and_absolute_paths(tmp_path: Path) -> None:
...
def test_resolve_workspace_path_can_require_existing_path(tmp_path: Path) -> None:
    with pytest.raises(FileNotFoundError):
        resolve_workspace_path(tmp_path, "missing.txt", must_exist=True)


def test_resolve_workspace_path_can_reject_workspace_root(tmp_path: Path) -> None:
...
def test_resolve_workspace_path_rejects_non_directory_workspace(tmp_path: Path) -> None:
