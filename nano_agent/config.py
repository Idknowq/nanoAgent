...
class AgentConfig(BaseModel):
    ...
    llm_model: str | None = "deepseek-v4-pro"  # 默认使用 DeepSeek 的高性能 Agent 模型。
    llm_temperature: float = Field(default=0.0, ge=0, le=2)  # 降低工具选择和补丁生成随机性。
    llm_max_output_tokens: int = Field(default=32_768, ge=1, le=384_000)  # 单次输出上限。
    llm_thinking_enabled: bool = False  # 当前消息协议未回传 reasoning_content，暂时禁用思考模式。
    llm_request_timeout_seconds: float = Field(default=300.0, gt=0, le=900)  # 单次 API 请求超时。
    max_steps: int = Field(default=160, ge=1)  # Agent 最大执行步数，覆盖复杂 SWE-bench 修复流程。
    command_timeout_seconds: int = Field(default=600, ge=1)  # 单个测试或构建命令的超时时间。
    ...
    max_file_bytes: int = Field(default=256_000, ge=1)  # 单个文件最多读取的字节数。
    stdout_tail_chars: int = Field(default=40_000, ge=1)  # stdout 最多保留的尾部字符数。
    stderr_tail_chars: int = Field(default=40_000, ge=1)  # stderr 最多保留的尾部字符数。
    ...
    snip_compact_ratio: float = Field(default=1.0, gt=0, le=1)  # 仅在自动摘要未充分降载时裁剪。
    snip_keep_head: int = Field(default=8, ge=1)  # snip 时保留的头部消息数。
    snip_keep_tail: int = Field(default=32, ge=1)  # snip 时保留的尾部消息数。
    micro_keep_recent_tool_results: int = Field(default=10, ge=0)  # 保留完整的最近工具结果数。
    micro_tool_result_min_chars: int = Field(default=4_000, ge=1)  # micro 压缩最小长度。
    context_max_input_tokens: int = Field(default=256_000, ge=1_000)  # Agent 输入 token 预算。
    context_auto_compact_ratio: float = Field(default=0.8, gt=0, le=1)  # 自动摘要阈值比例。
    context_output_reserve_tokens: int = Field(default=32_768, ge=0)  # 为模型输出保留的 token。
    max_auto_compactions: int = Field(default=3, ge=0, le=3)  # 单次 run 自动摘要上限。
    reactive_keep_recent_messages: int = Field(default=16, ge=1)  # 应急压缩保留的尾部消息数。
    max_reactive_compactions: int = Field(default=1, ge=0, le=1)  # 单次 run 应急压缩上限。
    llm_max_transient_retries: int = Field(default=4, ge=0, le=10)  # 临时 API 故障重试次数。
    llm_retry_base_seconds: float = Field(default=5.0, ge=0)  # 指数退避基础等待秒数。
    llm_retry_max_seconds: float = Field(default=60.0, ge=0)  # 本地退避最大等待秒数。
    ...
    subagent_max_result_chars: int = Field(default=16_000, ge=100)  # 回传主 Agent 的结果上限。
